...
const SelectViolationWeekPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchWeeks = async () => {
      try {
        const res = await api.get('/api/zalo/violation-weeks');
        if (res.data.success) {
  ...
  return (
    <Page className="bg-transparent">
      <Header variant="back" title="Chọn tuần - Nhập vi phạm" />
      <div className="px-4 pt-4 pb-32 space-y-3">
        {weeks.length === 0 ? (
          <div className="text-center py-10 text-slate-400">Không có tuần học nào được mở.</div>
        ) : (
          weeks.map((week) => {
            const hasDraft = week.draft_count > 0;
            const hasPending = week.pending_count > 0;

            return (
              <div 
                key={week.id}
                onClick={() => navigate(PATHS.VIOLATION_INPUT.replace(':id', week.id))}
              ...
              >
                <div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap mt-1">
                    ...
                    {hasDraft && (
                      <span className="px-2 py-0.5 text-[10px] font-bold rounded-md bg-orange-100 text-orange-600 uppercase">
                        {week.draft_count} Đang Nháp
                      </span>
                    )}
                    
                    {hasPending && (
                      <span className="px-2 py-0.5 text-[10px] font-bold rounded-md bg-blue-100 text-blue-600 uppercase">
                        {week.pending_count} Chờ Duyệt
                      </span>
                    )}
                  </div>
                </div>
...
export default SelectViolationWeekPage;
