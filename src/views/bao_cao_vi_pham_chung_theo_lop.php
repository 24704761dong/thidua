<?php
$page_title = 'Báo Cáo Vi Phạm Chung Theo Lớp';
...
$diem_tru_levels = $diem_tru_levels ?? [];
...
// Lấy danh sách các lớp để chuẩn bị bộ lọc
$unique_lops = [];
foreach ($report_data as $lop_name => $data) {
    if (!empty($lop_name) && !in_array($lop_name, $unique_lops)) {
        $unique_lops[] = $lop_name;
    }
}
sort($unique_lops);
?>
        ...
        <div class="px-5 py-3 border-b border-[#224397]/12 bg-[#f8fafc] flex flex-wrap items-end justify-between gap-4">
            ...
            <div class="flex flex-wrap items-end gap-3">
                ...
                <div>
                    ...
                    <input type="text" id="quick-search" class="form-input-sm min-w-[160px]" placeholder="Tên lớp...">
                </div>
                <?php endif; ?>
                </div>
            </div>
            ...
            <div id="actions-container" class="flex flex-wrap items-center gap-2">
                ...
                <?php if ($tuan_id): ?>
                <a href="/thidua/xuat-bao-cao-vi-pham-chung?tuan_id=<?php echo htmlspecialchars($tuan_id); ?>" class="btn-action-sm-green">
                    ...
                    <table class="log-table" id="reportTable">
                        <thead>
                            <tr>
                                ...
                                <th style="text-align: left; padding-left: 1rem; width:150px;">Lớp</th>
                                <th style="width:100px;">Sĩ Số</th>
                                <?php foreach ($diem_tru_levels as $diem): ?>
                                    <th>SL VP (<?php echo -$diem; ?>đ)</th>
                                <?php endforeach; ?>
                                <th style="width:120px;">Tổng SLVP</th>
                                <th style="width:140px;">Tổng Điểm Trừ</th>
                            </tr>
                        </thead>
                        <tbody id="report-tbody">
                            <?php if (empty($report_data)): ?>
                                <tr><td colspan="<?php echo 5 + count($diem_tru_levels); ?>" class="text-center p-6 text-slate-500 font-medium border-0">Không có dữ liệu vi phạm cho tuần này.</td></tr>
                            <?php else: ?>
                                <?php $stt_counter = 1; ?>
                                <?php foreach ($report_data as $lop_name => $data): 
                                    $khoi = get_grade_class($lop_name);
                                    $tong_diem_tru = 0;
                                ...
                                ?>
                                    <tr class="report-row" data-khoi="<?php echo $khoi; ?>" data-lop="<?php echo htmlspecialchars($lop_name); ?>">
                                        <td class="font-bold text-slate-400 row-stt"><?php echo $stt_counter++; ?></td>
                                        <td style="text-align: left; padding-left: 1rem;" class="font-bold text-[#224397]"><?php echo htmlspecialchars($data['ten_lop']); ?></td>
                                        <td class="text-slate-600 font-medium"><?php echo $data['si_so']; ?></td>
                                        <?php foreach ($diem_tru_levels as $diem):
                                            $so_luong = $data['vi_pham'][$diem] ?? 0;
                                        ?>
                                            <td class="text-slate-700 font-semibold"><?php if ($so_luong > 0) echo $so_luong; ?></td>
                                        <?php endforeach; ?>
                                        <td class="font-bold text-[#224397]"><?php if ($data['tong_so_luong'] > 0) echo $data['tong_so_luong']; ?></td>
                                        <td class="font-bold text-rose-600 bg-rose-50/20"><?php if ($tong_diem_tru > 0) echo -$tong_diem_tru; ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </tbody>
...
function initBaoCaoEvents() {
    ...
    function applyFilters() {
        ...
        if(summaryText) summaryText.innerHTML = `Hiển thị ${vis} / ${rows.length} lớp &middot; Đang dùng bộ lọc client`;
    }
...
async function loadTuanData(tuanId) {
    ...
    try {
        const url = `/thidua/bao-cao/vi-pham-chung-theo-lop?iframe=1&tuan_id=${tuanId}`;
