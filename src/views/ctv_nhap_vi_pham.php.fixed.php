      ...
      <table id="violation-table" class="w-full text-left text-sm text-slate-600">
        <thead>
          <tr>
            <th class="delete-col" style="width:36px">
              <input class="w-3.5 h-3.5 rounded border-slate-300" type="checkbox" id="select-all-checkbox" aria-label="Chn tất cả">
            </th>
            <th style="width:36px">STT</th>
            <th style="width:80px">Số CCCD</th>
            <th style="width:180px">H và Tên <span class="text-red-400">*</span></th>
            ...
            <th style="width:80px">Ngưi Nhập</th>
          </tr>
        </thead>
        <tbody id="violation-table-body">
          ...
          <tr data-vi-pham-id="<?= htmlspecialchars((string)($vp['id'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>"
              ...
              <?= (($vp['trang_thai_hoc_tap'] ?? '') === 'nghi_hoc') ? 'title="Hc sinh đã nghỉ hc"' : '' ?>>
            ...
            <td class="editable student-name relative p-0">
              ...
              <span class="status-dot" title="ã lưu"></span>
            </td>
            ...
            <td class="notification-status text-center">
              <?php
                ...
                if ($status === 'ã TB GV') {
                    echo '<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-sky-100 text-sky-800">ã TB GV</span>';
                } elseif ($status === 'ã TB HS') {
                    echo '<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-100 text-blue-800">ã TB HS</span>';
                } elseif ($status === 'ã TB') {
                    echo '<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-green-100 text-green-800">ã TB</span>';
                } else {
...
<div class="custom-modal-backdrop" id="modal-confirm-delete">
  <div class="custom-modal-box">
    ...
    <div class="custom-modal-msg" id="modal-confirm-delete-msg">Bạn có chắc chắn muốn xóa các mục đã chn?</div>
...
<div class="custom-modal-backdrop" id="modal-error">
  <div class="custom-modal-box">
    ...
    <div class="custom-modal-btns">
      <button class="btn-std btn-std-danger" onclick="closeModal('modal-error')">óng</button>
    </div>
  </div>
</div>
...
<div class="custom-modal-backdrop" id="confirmSendMailModal">
  <div class="custom-modal-box" style="max-width:500px">
    ...
    <div class="custom-modal-msg">ã lưu thành công! Bạn muốn gửi thông báo đến ai?</div>
    <div class="custom-modal-btns" style="flex-direction:column; gap:0.5rem">
      <div class="flex gap-2 justify-center">
        ...
        <button class="btn-std send-now-btn" data-target="hs" style="background:#8b5cf6;color:white;border-color:#8b5cf6;font-size:0.8rem;padding:0.5rem 1rem">Chỉ Hc sinh</button>
...
<div class="custom-modal-backdrop" id="sendStatusModal">
  <div class="custom-modal-box" style="max-width:560px; text-align:left">
    ...
    <div class="text-center">
      <button class="btn-std btn-std-primary" onclick="closeModal('sendStatusModal'); suppressBeforeUnload=true; window.location.reload();" style="font-size:0.85rem;padding:0.5rem 1.5rem">óng & Tải lại</button>
    </div>
  </div>
</div>
...
<div class="custom-modal-backdrop" id="sendTargetModal">
  <div class="custom-modal-box" style="max-width:480px">
    ...
    <div class="custom-modal-title text-[#224397]">Chn ối Tượng Gửi</div>
    <div class="custom-modal-msg">Chn đối tượng nhận thông báo email</div>
    <div class="custom-modal-btns">
      ...
      <button class="btn-std send-confirm-btn" data-target="hs" style="background:#8b5cf6;color:white;border-color:#8b5cf6">Chỉ Hc sinh</button>
  ...
  <div class="custom-modal-box has-header" style="max-width:520px; padding:0;">
    ...
    <form id="import-form" action="/thidua/admin/vi-pham?action=import" method="POST" enctype="multipart/form-data">
      <div class="custom-modal-body">
        ...
        <p class="text-sm text-slate-500 mb-3">Chn file Excel (.xlsx) theo đúng định dạng mẫu.</p>
...
<div class="custom-modal-backdrop" id="qr-scanner-modal">
  <div class="custom-modal-box has-header" style="max-width:400px; padding:0;">
    ...
    <div class="custom-modal-body text-center">
      ...
      <p class="mt-3 text-sm text-slate-500">ưa mã QR vào khung hình camera</p>
    </div>
  </div>
</div>
  ...
  <div class="custom-modal-box has-header" style="max-width:480px; padding:0;">
    ...
    <div class="custom-modal-body">
      <div id="admin-scan-info" class="hidden">
        ...
        <div class="mb-3">
          <label class="block text-xs font-semibold text-[#224397] uppercase mb-1">Chn Lỗi Vi Phạm</label>
        ...
        <div class="mb-3">
          <label class="block text-xs font-semibold text-[#224397] uppercase mb-1">Ghi Chú (tùy chn)</label>
...
<div class="custom-modal-backdrop" id="adminManualAddModal">
  <div class="custom-modal-box has-header" style="max-width:600px; padding:0;">
    ...
    <div class="custom-modal-body">
      <form id="admin-manual-add-form">
        <div class="grid grid-cols-2 gap-3 mb-3">
          <div>
            <label class="block text-xs font-semibold text-[#224397] uppercase mb-1">H và Tên <span class="text-red-400">*</span></label>
    ...
    <div class="custom-modal-footer">
      <button type="button" class="btn-std" onclick="closeModal('adminManualAddModal')">óng</button>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function sendNotifications(violation_ids, target) {
    ...
    try {
      ...
      if (result.success) {
        let reportHTML = '';
        if (result.sent_to_gvcn?.length) reportHTML += `<div class="mb-2"><strong class="text-green-700">ã gửi đến GVCN:</strong><ul class="mt-1 ml-4 text-sm">` + result.sent_to_gvcn.map(i => `<li>${i}</li>`).join('') + `</ul></div>`;
        if (result.sent_to_hs?.length) reportHTML += `<div class="mb-2"><strong class="text-green-700">ã gửi đến Hc sinh:</strong><ul class="mt-1 ml-4 text-sm">` + result.sent_to_hs.map(i => `<li>${i}</li>`).join('') + `</ul></div>`;
        ...
        if (result.failed_hs?.length) reportHTML += `<div class="mb-2"><strong class="text-red-700">Lỗi khi gửi đến Hc sinh:</strong><ul class="mt-1 ml-4 text-sm text-red-600">` + result.failed_hs.map(i => `<li>${i}</li>`).join('') + `</ul></div>`;
    ...
    try {
      ...
      if (data.success && data.student) {
        ...
        if (noteTd && noteDiv && (!normalizeSpaces(noteDiv.textContent) || noteTd.dataset.autoNote === '1')) {
          noteDiv.textContent = data.message || 'Không tìm thấy hc sinh.';
  ...
  saveAllBtn.addEventListener('click', async function () {
    this.disabled = true;
    this.innerHTML = '<span class="inline-block w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin mr-1"></span> ang quét...';
    ...
    if (!isValid) {
      showToast('Vui lòng sửa các ô bị lỗi (vin đ) trước khi lưu.', 'error');
      this.disabled = false;
      ...
      statusIndicator.textContent = 'ã dừng do có lỗi.'; return;
    }
    ...
    statusIndicator.textContent = `ang lưu ${violations.length} thay đổi...`;
    try {
      ...
      if (result.success) {
        ...
        if (newIds.length > 0) {
          ...
          showToast('ã lưu thành công!', 'success');
  ...
  document.querySelectorAll('.send-notification-btn').forEach(button => {
    button.addEventListener('click', function (e) {
      ...
      if (mode === 'all') {
        ...
        if (selected.length === 0) { showToast('Vui lòng chn ít nhất một vi phạm để gửi.', 'warning'); return; }
  ...
  if (confirmDeleteBtn) {
    confirmDeleteBtn.addEventListener('click', function () {
      ...
      if (idsToDelete.length === 0) { showToast('Vui lòng chn ít nhất một mục đã lưu để xóa.', 'warning'); return; }
      document.getElementById('modal-confirm-delete-msg').textContent = `Bạn có chắc chắn muốn xóa ${idsToDelete.length} mục đã chn? Hành động này không thể hoàn tác.`;
      document.getElementById('modal-confirm-delete-ok').onclick = async () => {
        ...
        try {
          ...
          const result = await response.json();
          loadingOverlay.style.display = 'none';
          if (result.success) { showToast(result.message || 'ã xóa thành công!', 'success'); setTimeout(() => { suppressBeforeUnload = true; window.location.reload(); }, 1000); }
  ...
  // Khoi dong
  tableBody.querySelectorAll('tr').forEach(row => {
    if (row.dataset.viPhamId) setRowStatus(row, 'saved', 'ã lưu');
  ...
  async function onScanSuccess(decodedText) {
    ...
    try {
      ...
      if (!response.ok) { const d = await response.json(); throw new Error(d.message || 'Hc sinh không tồn tại.'); }
  ...
  startScanBtn.addEventListener('click', () => {
    ...
    html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } }, onScanSuccess, () => {})
      .catch(() => { showToast('Không thể khởi động camera. Vui lòng cấp quyn truy cập camera.', 'error'); closeScannerModal(); });
  });
  ...
  document.getElementById('admin-confirm-violation-btn-qr').addEventListener('click', function () {
    ...
    if (!violationId) { showToast('Vui lòng chn một lỗi vi phạm.', 'warning'); return; }
  ...
  const manualLookupStudent = async () => {
    ...
    manualLookupResult.innerHTML = `<div class="flex items-center text-slate-500 text-sm gap-2"><span class="inline-block w-3 h-3 border-2 border-[#224397] border-t-transparent rounded-full animate-spin"></span>ang tìm...</div>`;
  ...
  manualConfirmBtn.addEventListener('click', function () {
    if (!lookedUpStudentData) { showToast('Vui lòng tìm hc sinh hợp lệ.', 'warning'); return; }
    ...
    showToast('ã thêm vào bảng! Nhớ lưu để ghi vào cơ sở dữ liệu.', 'success');
  });
    // X l gi vi phm cho admin
    const sendToAdminBtn = document.getElementById('send-to-admin-btn');
    if (sendToAdminBtn) {
      sendToAdminBtn.addEventListener('click', async function() {
        if (confirm('Bn c chc mun gi cc mc nhp ny cho Admin duyt?')) {
            this.disabled = true;
            this.innerHTML = '<span class="spinner-border spinner-border-sm" style="width: 1rem; height: 1rem; border-width: 0.15em;"></span> ang gi...';
            try {
            ...
            } catch (err) { alert('Li kt ni!'); }
            finally {
                this.disabled = false;
                this.innerHTML = '<i class="bi bi-send-check mr-1"></i>Gi duyt';
            }
        }
      });
    }
