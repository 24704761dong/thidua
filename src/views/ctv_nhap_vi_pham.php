...
?>
<?php
$page_title = 'Nhập Vi Phạm - ' . htmlspecialchars($tuan_hoc['ten_tuan'] ?? '', ENT_QUOTES, 'UTF-8');
...
body::-webkit-scrollbar-thumb { background: rgba(34,67,151,0.3) !important; border-radius: 4px; }
body::-webkit-scrollbar-track { background: transparent !important; }
body, html { scrollbar-width: auto !important; -ms-overflow-style: auto !important; }
...
<div class="w-full px-2 lg:px-4">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center gap-2 mb-3">
    ...
    <div class="flex items-center gap-2 flex-1">
      <div class="bg-[#224397]/8 border border-[#224397]/20 rounded px-3 py-1.5 flex items-center gap-2">
        ...
        <span class="font-bold text-[#224397] text-[12px]">NHẬP VI PHẠM:</span>
    ...
    <div class="flex items-center gap-1.5 flex-wrap" id="toolbar-normal">
      ...
      <!-- Quay lai -->
      <a href="/thidua/hocsinh/chon-tuan" class="btn-std">
      ...
      <!-- Gui vi pham -->
      <button id="send-to-admin-btn" class="btn-std btn-std-primary" style="background:#16a34a;border-color:#15803d;">
        <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" fill="currentColor" class="bi bi-send-check" viewBox="0 0 16 16"><path d="M15.964.686a.5.5 0 0 0-.65-.65L.767 5.855H.766l-.452.18a.5.5 0 0 0-.082.887l.41.26.001.002 4.995 3.178 1.59 2.498C8 14 8 13 8 12.5a4.5 4.5 0 0 1 5.026-4.47zm-1.833 1.89L6.637 10.07l-.215-.338a.5.5 0 0 0-.154-.154l-.338-.215 7.494-7.494 1.178-.471z"/><path d="M16 12.5a3.5 3.5 0 1 1-7 0 3.5 3.5 0 0 1 7 0m-1.993-1.679a.5.5 0 0 0-.686.172l-1.17 1.95-.547-.547a.5.5 0 0 0-.708.708l.774.773a.75.75 0 0 0 1.174-.144l1.335-2.226a.5.5 0 0 0-.172-.686"/></svg>
        G?i duy?t
      </button>
    </div>
  </div>
  ...
  <!-- Phim tat hint -->
  <div class="text-[10px] text-slate-400 mb-2 italic">Mẹo: Ctrl/Cmd+S để lưu nhanh | Arrow keys để di chuyển | Paste Excel trực tiếp vào bảng</div>
  ...
  <div class="bg-white rounded shadow border border-[#224397]/25 mb-4 p-0 overflow-hidden">
    <div class="px-4 py-3 border-b border-[#224397]/20 bg-[#224397]/5 flex items-center justify-between">
      ...
      <span class="text-[11px] text-slate-400"><?= count($danh_sach_vi_pham_da_nhap ?? []) ?> bản ghi</span>
    </div>
      ...
      <table id="violation-table" class="w-full text-left text-sm text-slate-600">
        <thead>
          <tr>
            <th class="delete-col" style="width:36px">
              <input class="w-3.5 h-3.5 rounded border-slate-300" type="checkbox" id="select-all-checkbox" aria-label="Chọn tất cả">
            </th>
            <th style="width:36px">STT</th>
            <th style="width:80px">Số CCCD</th>
            <th style="width:180px">Họ và Tên <span class="text-red-400">*</span></th>
            <th style="width:70px">Lớp <span class="text-red-400">*</span></th>
            <th style="width:95px">Ngày Vi Phạm <span class="text-red-400">*</span></th>
            <th>Tên Vi Phạm <span class="text-red-400">*</span></th>
            <th style="width:150px">Ghi Chú</th>
            <th style="width:80px">Trạng Thái</th>
            <th style="width:80px">Người Nhập</th>
          </tr>
        </thead>
        <tbody id="violation-table-body">
          ...
          <tr data-vi-pham-id="<?= htmlspecialchars((string)($vp['id'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>"
              ...
              <?= (($vp['trang_thai_hoc_tap'] ?? '') === 'nghi_hoc') ? 'title="Học sinh đã nghỉ học"' : '' ?>>
            ...
            <td class="editable student-name relative p-0">
              ...
              <span class="status-dot" title="Đã lưu"></span>
            </td>
            ...
            <td class="violation-name-cell relative p-0">
              <input type="text" class="cell-edit violation-name" list="violation-list-datalist" value="<?= htmlspecialchars($vp['ten_vi_pham'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" placeholder="Gõ để tìm...">
            </td>
            ...
            <td class="notification-status text-center">
              <?php
                $status = $vp['trang_thai_thong_bao'] ?? 'Chưa TB';
                if ($status === 'Đã TB GV') {
                    echo '<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-sky-100 text-sky-800">Đã TB GV</span>';
                } elseif ($status === 'Đã TB HS') {
                    echo '<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-100 text-blue-800">Đã TB HS</span>';
                } elseif ($status === 'Đã TB') {
                    echo '<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-green-100 text-green-800">Đã TB</span>';
                } else {
                    echo '<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-slate-100 text-slate-600">Chưa TB</span>';
                }
              ?>
            </td>
...
<template id="new-row-template">
  <tr data-status="new">
    ...
    <td class="editable student-name relative p-0"><div contenteditable="true" class="cell-edit"></div><span class="status-dot" title="Hàng mới"></span></td>
    ...
    <td class="violation-name-cell relative p-0"><input type="text" class="cell-edit violation-name" list="violation-list-datalist" placeholder="Gõ để tìm..."></td>
    <td class="editable note relative p-0"><div contenteditable="true" class="cell-edit"></div></td>
    <td class="notification-status text-center"><span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-slate-100 text-slate-600">Chưa TB</span></td>
...
<div class="custom-modal-backdrop" id="modal-confirm-delete">
  <div class="custom-modal-box">
    ...
    <div class="custom-modal-title text-red-600">Cảnh Báo Xóa!</div>
    <div class="custom-modal-msg" id="modal-confirm-delete-msg">Bạn có chắc chắn muốn xóa các mục đã chọn?</div>
    <div class="custom-modal-btns">
      <button class="btn-std btn-std-danger" id="modal-confirm-delete-ok">Xác nhận Xóa</button>
      <button class="btn-std" onclick="closeModal('modal-confirm-delete')">Hủy</button>
    </div>
  </div>
</div>
...
<div class="custom-modal-backdrop" id="modal-success">
  <div class="custom-modal-box">
    ...
    <div class="custom-modal-title text-[#224397]" id="modal-success-title">Thành công!</div>
...
<div class="custom-modal-backdrop" id="modal-error">
  <div class="custom-modal-box">
    ...
    <div class="custom-modal-title text-red-600">Có Lỗi Xảy Ra</div>
    <div class="custom-modal-msg" id="modal-error-msg"></div>
    <div class="custom-modal-btns">
      <button class="btn-std btn-std-danger" onclick="closeModal('modal-error')">Đóng</button>
    </div>
  </div>
</div>
...
<div class="custom-modal-backdrop" id="confirmSendMailModal">
  <div class="custom-modal-box" style="max-width:500px">
    ...
    <div class="custom-modal-title text-[#224397]">Gửi Thông Báo Email</div>
    <div class="custom-modal-msg">Đã lưu thành công! Bạn muốn gửi thông báo đến ai?</div>
    <div class="custom-modal-btns" style="flex-direction:column; gap:0.5rem">
      <div class="flex gap-2 justify-center">
        <button class="btn-std send-now-btn" data-target="gvcn" style="background:#0ea5e9;color:white;border-color:#0ea5e9;font-size:0.8rem;padding:0.5rem 1rem">Chỉ GVCN</button>
        <button class="btn-std send-now-btn" data-target="hs" style="background:#8b5cf6;color:white;border-color:#8b5cf6;font-size:0.8rem;padding:0.5rem 1rem">Chỉ Học sinh</button>
        <button class="btn-std send-now-btn" data-target="both" style="background:#16a34a;color:white;border-color:#16a34a;font-size:0.8rem;padding:0.5rem 1rem">Cả hai</button>
      </div>
      <button class="btn-std" onclick="closeModal('confirmSendMailModal'); suppressBeforeUnload=true; window.location.reload();" style="align-self:center">Không gửi, tải lại trang</button>
    </div>
  </div>
</div>
...
<div class="custom-modal-backdrop" id="sendStatusModal">
  <div class="custom-modal-box" style="max-width:560px; text-align:left">
    <div class="text-center mb-4">
      ...
      <div class="custom-modal-title text-[#224397]">Kết Quả Gửi Email</div>
    </div>
    <div id="sendStatusModalBody" class="text-sm text-slate-600 max-h-64 overflow-y-auto mb-4"></div>
    <div class="text-center">
      <button class="btn-std btn-std-primary" onclick="closeModal('sendStatusModal'); suppressBeforeUnload=true; window.location.reload();" style="font-size:0.85rem;padding:0.5rem 1.5rem">Đóng & Tải lại</button>
    </div>
  </div>
</div>
...
<div class="custom-modal-backdrop" id="sendTargetModal">
  <div class="custom-modal-box" style="max-width:480px">
    ...
    <div class="custom-modal-title text-[#224397]">Chọn Äá»‘i Tượng Gửi</div>
    <div class="custom-modal-msg">Chọn đối tượng nhận thông báo email</div>
    <div class="custom-modal-btns">
      <button class="btn-std send-confirm-btn" data-target="gvcn" style="background:#0ea5e9;color:white;border-color:#0ea5e9">Chỉ GVCN</button>
      <button class="btn-std send-confirm-btn" data-target="hs" style="background:#8b5cf6;color:white;border-color:#8b5cf6">Chỉ Học sinh</button>
      <button class="btn-std send-confirm-btn" data-target="both" style="background:#16a34a;color:white;border-color:#16a34a">Cả hai</button>
      <button class="btn-std" onclick="closeModal('sendTargetModal')">Hủy</button>
    </div>
  </div>
</div>
  ...
  <div class="custom-modal-box has-header" style="max-width:520px; padding:0;">
    ...
    <form id="import-form" action="/thidua/admin/vi-pham?action=import" method="POST" enctype="multipart/form-data">
      <div class="custom-modal-body">
        ...
        <p class="text-sm text-slate-500 mb-3">Chọn file Excel (.xlsx) theo đúng định dạng mẫu.</p>
        ...
        <div class="bg-[#224397]/5 rounded-lg p-3 text-xs text-slate-500 flex items-start gap-2">
          ...
          Bạn có thể tải file mẫu ở menu <strong>Excel &rarr; Tải File Mẫu</strong>.
        </div>
      </div>
      <div class="custom-modal-footer">
        <button type="button" class="btn-std" onclick="closeModal('importModal')">Hủy</button>
        <button type="submit" class="btn-std btn-std-primary">Tải lên</button>
      </div>
    </form>
  </div>
</div>
...
<div class="custom-modal-backdrop" id="qr-scanner-modal">
  <div class="custom-modal-box has-header" style="max-width:400px; padding:0;">
    <div class="custom-modal-header">
      <h5><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" class="bi bi-qr-code-scan mr-2 inline-block" viewBox="0 0 16 16"><path d="M0 .5A.5.5 0 0 1 .5 0h3a.5.5 0 0 1 0 1H1v2.5a.5.5 0 0 1-1 0zm12 0a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v3a.5.5 0 0 1-1 0V1h-2.5a.5.5 0 0 1-.5-.5M.5 12a.5.5 0 0 1 .5.5V15h2.5a.5.5 0 0 1 0 1h-3a.5.5 0 0 1-.5-.5v-3a.5.5 0 0 1 .5-.5m15 0a.5.5 0 0 1 .5.5v3a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1 0-1H15v-2.5a.5.5 0 0 1 .5-.5M4 4h1v1H4z"/><path d="M7 2H2v5h5zM3 3h3v3H3zm2 8H4v1h1z"/><path d="M7 9H2v5h5zm-4 1h3v3H3zm8-6h1v1h-1z"/><path d="M9 2h5v5H9zm1 1v3h3V3zM8 8h1v1H8zm2 0h1v1h-1zm-1 2h1v1H9zm2 0h1v1h-1zm-3 1h1v1H8zm4 1h1v1h-1zm-3 0h1v1h-1zm3-3h1v1h-1z"/></svg> Quét Mã QR</h5>
    ...
    <div class="custom-modal-body text-center">
      ...
      <p class="mt-3 text-sm text-slate-500">Đưa mã QR vào khung hình camera</p>
    </div>
  </div>
</div>
  ...
  <div class="custom-modal-box has-header" style="max-width:480px; padding:0;">
    ...
    <div class="custom-modal-body">
      <div id="admin-scan-info" class="hidden">
        ...
        <div class="mb-3">
          <label class="block text-xs font-semibold text-[#224397] uppercase mb-1">Chọn Lỗi Vi Phạm</label>
        ...
        <div class="mb-3">
          <label class="block text-xs font-semibold text-[#224397] uppercase mb-1">Ghi Chú (tùy chọn)</label>
          <input type="text" id="admin-violation-notes-qr" class="w-full border border-[#224397]/30 rounded-lg px-3 py-2 text-sm focus:border-[#224397] focus:ring-1 focus:ring-[#224397]" placeholder="Ghi chú thêm...">
        </div>
      </div>
      <div id="admin-scan-error" class="hidden p-3 bg-red-50 border border-red-200 rounded-lg text-red-800 text-sm"></div>
    </div>
    <div class="custom-modal-footer">
      <button id="admin-cancel-scan-btn" class="btn-std">Quét lại</button>
      <button id="admin-confirm-violation-btn-qr" class="btn-std btn-std-primary">Xác Nhận & Thêm</button>
    </div>
  </div>
</div>
...
<div class="custom-modal-backdrop" id="adminManualAddModal">
  <div class="custom-modal-box has-header" style="max-width:600px; padding:0;">
    <div class="custom-modal-header">
      <h5><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" class="bi bi-keyboard mr-2 inline-block" viewBox="0 0 16 16"><path d="M14 5a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1zM2 4a2 2 0 0 0-2 2v5a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2z"/><path d="M13 10.25a.25.25 0 0 1 .25-.25h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5a.25.25 0 0 1-.25-.25zm0-2a.25.25 0 0 1 .25-.25h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5a.25.25 0 0 1-.25-.25zm-5 0A.25.25 0 0 1 8.25 8h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5A.25.25 0 0 1 8 8.75zm2 0a.25.25 0 0 1 .25-.25h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5a.25.25 0 0 1-.25-.25zm2 0a.25.25 0 0 1 .25-.25h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5a.25.25 0 0 1-.25-.25zm2 0a.25.25 0 0 1 .25-.25h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5a.25.25 0 0 1-.25-.25zm-6 2A.25.25 0 0 1 6.25 10h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5A.25.25 0 0 1 6 10.75zm2 0a.25.25 0 0 1 .25-.25h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5a.25.25 0 0 1-.25-.25zm2 0a.25.25 0 0 1 .25-.25h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5A.25.25 0 0 1 8 10.75zm2 0a.25.25 0 0 1 .25-.25v.5a.25.25 0 0 1-.25.25h-.5A.25.25 0 0 1 10 10.75zm2 0a.25.25 0 0 1 .25-.25v.5a.25.25 0 0 1-.25.25h-.5a.25.25 0 0 1-.25-.25v-.5A.25.25 0 0 1 12.25 10zM2.25 8h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5A.25.25 0 0 1 2 8.75v-.5A.25.25 0 0 1 2.25 8zm2 0h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5A.25.25 0 0 1 4 8.75v-.5A.25.25 0 0 1 4.25 8zm2 0h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5A.25.25 0 0 1 6 8.75v-.5A.25.25 0 0 1 6.25 8zM2.25 10h.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-.5A.25.25 0 0 1 2 10.75v-.5A.25.25 0 0 1 2.25 10zm4.5 0h4.5a.25.25 0 0 1 .25.25v.5a.25.25 0 0 1-.25.25h-4.5A.25.25 0 0 1 6.5 10.75v-.5A.25.25 0 0 1 6.75 10z"/></svg> Nhập Tay Vi Phạm</h5>
    ...
    <div class="custom-modal-body">
      <form id="admin-manual-add-form">
        <div class="grid grid-cols-2 gap-3 mb-3">
          <div>
            <label class="block text-xs font-semibold text-[#224397] uppercase mb-1">Họ và Tên <span class="text-red-400">*</span></label>
          ...
          <div>
            <label class="block text-xs font-semibold text-[#224397] uppercase mb-1">Lớp <span class="text-red-400">*</span></label>
        ...
        <div class="grid grid-cols-2 gap-3 mb-3">
          <div>
            <label class="block text-xs font-semibold text-[#224397] uppercase mb-1">Ngày Vi Phạm <span class="text-red-400">*</span></label>
          ...
          <div>
            <label class="block text-xs font-semibold text-[#224397] uppercase mb-1">Lỗi Vi Phạm <span class="text-red-400">*</span></label>
        ...
        <div>
          <label class="block text-xs font-semibold text-[#224397] uppercase mb-1">Ghi Chú</label>
          <input type="text" id="admin-violation-notes-manual" class="w-full border border-[#224397]/30 rounded-lg px-3 py-2 text-sm focus:border-[#224397] focus:ring-1 focus:ring-[#224397]" placeholder="Ghi chú thêm...">
        </div>
      </form>
    </div>
    <div class="custom-modal-footer">
      <button type="button" class="btn-std" onclick="closeModal('adminManualAddModal')">Đóng</button>
      <button id="admin-confirm-violation-btn-manual" class="btn-std btn-std-primary" disabled>Thêm Vào Bảng</button>
    </div>
  </div>
</div>
...
function showSuccess(msg, title = 'Thành công!', cb = null) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function sendNotifications(violation_ids, target) {
    if (!violation_ids || violation_ids.length === 0) {
      showToast('Không có vi phạm nào để gửi thông báo.', 'warning');
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        let reportHTML = '';
        if (result.sent_to_gvcn?.length) reportHTML += `<div class="mb-2"><strong class="text-green-700">Đã gửi đến GVCN:</strong><ul class="mt-1 ml-4 text-sm">` + result.sent_to_gvcn.map(i => `<li>${i}</li>`).join('') + `</ul></div>`;
        if (result.sent_to_hs?.length) reportHTML += `<div class="mb-2"><strong class="text-green-700">Đã gửi đến Học sinh:</strong><ul class="mt-1 ml-4 text-sm">` + result.sent_to_hs.map(i => `<li>${i}</li>`).join('') + `</ul></div>`;
        if (result.failed_gvcn?.length) reportHTML += `<div class="mb-2"><strong class="text-red-700">Lỗi khi gửi đến GVCN:</strong><ul class="mt-1 ml-4 text-sm text-red-600">` + result.failed_gvcn.map(i => `<li>${i}</li>`).join('') + `</ul></div>`;
        if (result.failed_hs?.length) reportHTML += `<div class="mb-2"><strong class="text-red-700">Lỗi khi gửi đến Học sinh:</strong><ul class="mt-1 ml-4 text-sm text-red-600">` + result.failed_hs.map(i => `<li>${i}</li>`).join('') + `</ul></div>`;
        document.getElementById('sendStatusModalBody').innerHTML = reportHTML || '<p class="text-center text-slate-500">Không có email nào được gửi đi.</p>';
        openModal('sendStatusModal');
      } else {
        throw new Error(result.message || 'Gửi thông báo thất bại.');
      }
    } catch (error) {
      showError('Lỗi khi gửi thông báo: ' + error.message);
    } finally {
    ...
    try {
      ...
      if (data.success && data.student) {
        ...
        if (noteTd && noteDiv && (!normalizeSpaces(noteDiv.textContent) || noteTd.dataset.autoNote === '1')) {
          noteDiv.textContent = data.message || 'Không tìm thấy học sinh.';
          noteTd.dataset.autoNote = '1';
        }
      }
    } catch (e) {
      if (e.name === 'AbortError') return;
      codeCell.innerHTML = '<span class="text-red-600">Lỗi API</span>';
    }
  }
  ...
  function addNewRow({ focus = true, status = 'new' } = {}) {
    ...
    if (dot) ensureTooltip(dot, 'Hàng mới');
  ...
  tableBody.addEventListener('blur', function (e) {
    ...
    if (cell.matches('.cell-edit') || cell.matches('.violation-name')) {
      ...
      if (current !== (cell.dataset.initialValue || '') && row.dataset.status === 'saved') setRowStatus(row, 'modified', 'Dữ liệu đã thay đổi');
  ...
  tableBody.addEventListener('input', function (e) {
    ...
    if (row.dataset.status === 'saved' && (e.target.closest('.editable') || e.target.classList.contains('violation-name'))) setRowStatus(row, 'modified', 'Dữ liệu đã thay đổi');
  ...
  saveAllBtn.addEventListener('click', async function () {
    this.disabled = true;
    this.innerHTML = '<span class="inline-block w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin mr-1"></span> Đang quét...';
    statusIndicator.textContent = 'Bắt đầu quét...'; statusIndicator.classList.remove('hidden');
    ...
    if (!isValid) {
      showToast('Vui lòng sửa các ô bị lỗi (viền Ä‘ỏ) trước khi lưu.', 'error');
      this.disabled = false;
      this.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" fill="currentColor" class="bi bi-save" viewBox="0 0 16 16"><path d="M2 1a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H9.5a1 1 0 0 0-1 1v7.293l2.646-2.647a.5.5 0 0 1 .708.708l-3.5 3.5a.5.5 0 0 1-.708 0l-3.5-3.5a.5.5 0 1 1 .708-.708L7.5 9.293V2a2 2 0 0 1 2-2H14a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V2a2 2 0 0 1 2-2h2.5a.5.5 0 0 1 0 1z"/></svg> Lưu tất cả';
      statusIndicator.textContent = 'Đã dừng do có lỗi.'; return;
    }
    ...
    statusIndicator.textContent = `Đang lưu ${violations.length} thay đổi...`;
    try {
      const response = await fetch('/thidua/api/ctv-luu-nhieu-vi-pham', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({violations}) });
      const result = await response.json();
      if (result.success) {
        ...
        if (newIds.length > 0) {
          ...
          document.getElementById('confirmSendMailModal').addEventListener('click', function h(e) {
            if (e.target.textContent.includes('Không gửi')) { suppressBeforeUnload = true; window.location.reload(); this.removeEventListener('click', h); }
          });
    ...
    button.addEventListener('click', function (e) {
      ...
      if (mode === 'all') {
        ...
        tableBody.querySelectorAll('tr').forEach(row => {
          ...
          if (badge && badge.textContent.includes('Chưa')) { const id = row.dataset.viPhamId; if (id) ids.push(id); }
        });
      } else if (mode === 'selected') {
        const selected = tableBody.querySelectorAll('.row-checkbox:checked');
        if (selected.length === 0) { showToast('Vui lòng chọn ít nhất một vi phạm để gửi.', 'warning'); return; }
        selected.forEach(cb => ids.push(cb.value));
      }
      if (ids.length === 0) { showToast('Không tìm thấy vi phạm nào phù hợp.', 'warning'); return; }
      idsForSending = ids;
  ...
  if (confirmDeleteBtn) {
    confirmDeleteBtn.addEventListener('click', function () {
      ...
      if (idsToDelete.length === 0) { showToast('Vui lòng chọn ít nhất một mục đã lưu để xóa.', 'warning'); return; }
      document.getElementById('modal-confirm-delete-msg').textContent = `Bạn có chắc chắn muốn xóa ${idsToDelete.length} mục đã chọn? Hành động này không thể hoàn tác.`;
      document.getElementById('modal-confirm-delete-ok').onclick = async () => {
        closeModal('modal-confirm-delete');
        loadingOverlay.style.display = 'flex';
        try {
          const response = await fetch('/thidua/api/ctv-xoa-nhieu-vi-pham', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ids: idsToDelete}) });
          const result = await response.json();
          loadingOverlay.style.display = 'none';
          if (result.success) { showToast(result.message || 'Đã xóa thành công!', 'success'); setTimeout(() => { suppressBeforeUnload = true; window.location.reload(); }, 1000); }
          else showError(result.message || 'Xóa thất bại.');
        } catch (error) {
          loadingOverlay.style.display = 'none';
  ...
  // Khoi dong
  tableBody.querySelectorAll('tr').forEach(row => {
    if (row.dataset.viPhamId) setRowStatus(row, 'saved', 'Đã lưu');
    else setRowStatus(row, 'new', 'Hàng mới');
  ...
  function addDataToGrid(studentData, violationId, violationName, date, notes) {
    ...
    setRowStatus(newRow, 'new', 'Hàng mới');
  ...
  async function onScanSuccess(decodedText) {
    ...
    try {
      ...
      if (!response.ok) { const d = await response.json(); throw new Error(d.message || 'Học sinh không tồn tại.'); }
  ...
  startScanBtn.addEventListener('click', () => {
    ...
    html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } }, onScanSuccess, () => {})
      .catch(() => { showToast('Không thể khởi động camera. Vui lòng cấp quyền truy cập camera.', 'error'); closeScannerModal(); });
  });
  ...
  document.getElementById('admin-confirm-violation-btn-qr').addEventListener('click', function () {
    ...
    if (!violationId) { showToast('Vui lòng chọn một lỗi vi phạm.', 'warning'); return; }
  ...
  const manualLookupStudent = async () => {
    ...
    manualLookupResult.innerHTML = `<div class="flex items-center text-slate-500 text-sm gap-2"><span class="inline-block w-3 h-3 border-2 border-[#224397] border-t-transparent rounded-full animate-spin"></span>Đang tìm...</div>`;
    ...
    try {
      ...
      } else throw new Error(data.message || 'Không tìm thấy thông tin.');
    } catch (error) {
  ...
  manualConfirmBtn.addEventListener('click', function () {
    if (!lookedUpStudentData) { showToast('Vui lòng tìm học sinh hợp lệ.', 'warning'); return; }
    ...
    showToast('Đã thêm vào bảng! Nhớ lưu để ghi vào cơ sở dữ liệu.', 'success');
  });
    // Xử lý gửi vi phạm cho admin
    const sendToAdminBtn = document.getElementById('send-to-admin-btn');
    if (sendToAdminBtn) {
      sendToAdminBtn.addEventListener('click', async function() {
        if (confirm('Bạn có chắc muốn gửi các mục nháp này cho Admin duyệt?')) {
            this.disabled = true;
            this.innerHTML = '<span class="spinner-border spinner-border-sm" style="width: 1rem; height: 1rem; border-width: 0.15em;"></span> Đang gửi...';
            try {
                const response = await fetch('/thidua/api/ctv-submit-violations', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ tuan_id: tuanHocId })
                });
                const result = await response.json();
                ...
                this.disabled = false;
                this.innerHTML = '<i class="bi bi-send-check mr-1"></i>Gửi duyệt';
            }
        }
      });
    }
