<?php
// File: src/controllers/api_backup_single_onedrive.php
...
// Quá trình sao lưu OneDrive mất rất nhiều thời gian (tải file, gọi API, v.v.)
// Nếu không đóng session, mọi request khác của user này sẽ bị block cho đến khi backup xong (gây lag)
session_write_close();
...
try {
    ...
    if (!$file) {
        throw new Exception("Không tìm thấy minh chứng.");
    }
    ...
    if ($file['storage_driver'] === 'cloud' || $file['storage_driver'] === 'r2') {
        ...
        // Download from R2
        $storage->downloadToPath($file['cloud_key'], $tempFile);
    ...
    $safe_tuan = preg_replace('/[^a-zA-Z0-9_\-\x{00A0}-\x{FFFF} \.]/u', '_', $file['ten_tuan'] ?? 'Khong_xac_dinh');
    $safe_lop = preg_replace('/[^a-zA-Z0-9_\-\x{00A0}-\x{FFFF} \.]/u', '_', $file['ten_lop'] ?? 'Khong_xac_dinh');
    $safe_name = preg_replace('/[^a-zA-Z0-9_\-\x{00A0}-\x{FFFF} \.]/u', '_', $file['original_filename']);
    ...
    // Path trên OneDrive: Thu_Vien_Minh_Chung/Tuan_.../Lop_.../filename
    $onedrivePath = "Thu_Vien_Minh_Chung/{$safe_tuan}/{$safe_lop}/{$safe_name}";
    
    $onedriveId = uploadToOneDrive($token, $ms_email, $onedrivePath, $localPath);

    // 4. Cập nhật Database
    $stmtUpdate = $db->prepare("UPDATE so_nhat_ky_minh_chung SET storage_driver = 'onedrive', cloud_key = ? WHERE id = ?");
    $stmtUpdate->execute([$onedriveId, $id]);
    ...
    if (!empty($file['thumbnail_path'])) {
        $thumbLocalPath = $base_path . '/' . ltrim($file['thumbnail_path'], '/');
        if (file_exists($thumbLocalPath)) {
            @unlink($thumbLocalPath);
        }
    }
    
    // (Tuỳ chọn) Nếu trước đó là R2, có thể xoá trên R2 luôn. Hiện tại ta giữ R2 hoặc xoá?
    if ($file['storage_driver'] === 'cloud' || $file['storage_driver'] === 'r2') {
        try {
            $storage = new StorageService();
            $storage->delete($file['cloud_key']);
        } catch (Exception $e) {
